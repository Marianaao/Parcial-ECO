...
function mouseReleased() {
    ...
    if (!buttonReleased(leftButton) && !buttonReleased(rightButton)) {
        ...
        1) Emit a message when the user is not tapping at any button tenemos que especificar que no se mueve en centro
        _____________________________________________ */
        socket.emit ("instruction-controller", message =>{
            message.screen++;
...
let directionController = ""; //la declaro


function sendDirection(direction, posX, posY) {
    if(dist(pmouseX, pmouseY, posX, posY))
